...
class CenterPredictorBEV(nn.Module):
    def __init__(self, inplanes=32, channel=256, img_sz=200, freeze_bn=False):
        super(CenterPredictorBEV, self).__init__()
        self.input_dim = channel
        self.img_sz = img_sz
    ...
    def forward(self, x, gt_score_map=None):
        """ Forward pass with input x. x.shape:(B, C, H, W) """
        score_map_ctr = self.get_score_map(x)
        return score_map_ctr
