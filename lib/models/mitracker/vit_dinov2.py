...
from lib.models.mitracker.base_backbone import BaseBackbone
...
class Attention(nn.Module):
    ...
    def forward(self, x, lens_z, lens_x, return_attention=False):
        if self.attn_type == 'concat':
            B, N, C = x.shape
            # x: (B, N, C) -> qkv: (B, N, 3, num_heads, C//num_heads) -> q, k, v: (B, num_heads, N, C//num_heads)
            qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
            q, k, v = qkv[0], qkv[1], qkv[2]  # make torchscript happy (cannot use tensor as tuple)

            attn = (q @ k.transpose(-2, -1)) * self.scale # atten: (B, num_heads, N, N)
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, attn_type="concat",ffn_layer='mlp',layer_scale=False, init_values=None):
        super().__init__()
...
class DinoVisionTransformer(BaseBackbone):
    """ Vision Transformer
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 act_layer=None, weight_init='', add_cls_token=False, attn_type="concat", ffn_layer='mlp',layer_scale=True,init_values = None,):
        """
        Args:
        ...
        self.blocks = nn.Sequential(*[
            Block(
                ...
                attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, init_values=init_values,
                act_layer=act_layer, attn_type=attn_type,ffn_layer=ffn_layer,layer_scale=layer_scale)
...
@torch.no_grad()
def _load_weights(model: DinoVisionTransformer, checkpoint_path: str, prefix: str = ''):
    ...
    if pretrained:
        if 'npz' in pretrained:
            ...
            try:
                ...
                print("Warning: DINOv2 Pretrained model weights are not loaded !")

    return model


def vit_base_patch14_518_dinov2(pretrained=False, **kwargs):
    """
    ViT-Base model (ViT-B/16) from original paper (https://arxiv.org/abs/2010.11929).
    """
    model_kwargs = dict(
        img_size = 518, patch_size=14, embed_dim=768, depth=12, num_heads=12, ffn_layer='mlp', init_values = 1e-5, **kwargs)
    model = _create_vision_transformer(pretrained=pretrained, **model_kwargs)
    return model


def vit_large_patch14_518_dinov2(pretrained=False, **kwargs):
    """ ViT-Large model (ViT-L/16) from original paper (https://arxiv.org/abs/2010.11929).
    """
    model_kwargs = dict(
        img_size = 518, patch_size=14, embed_dim=1024, depth=24, num_heads=16, ffn_layer='mlp', **kwargs)
    model = _create_vision_transformer(pretrained=pretrained, **model_kwargs)
    return model