...
from lib.models.mitracker.utils import combine_tokens, recover_tokens


class BaseBackbone(nn.Module): # designed for tracking
    def __init__(self):
        super().__init__()
        self.pos_embed = None
    ...
    def finetune_track(self, cfg, patch_start_index=1):
        ...
        patch_pos_embed = self.pos_embed[:, patch_start_index:, :] # remove cls token, (B, N, C)
    ...
    def forward_features(self, z, x, track_query=None, token_type="add"):
        '''
        z (torch.Tensor): template feature, [B, C, H_z, W_z]*T_z
        ...
        if isinstance(z, list):
            z = torch.stack(z, dim=1) # [B, C, H_z, W_z]*T_z -> [B, T_z, C, H_z, W_z]
        elif isinstance(z, torch.Tensor):
            z = z.transpose(0,1) # [T_z, B, C, H_z, W_z] -> [B, T_z, C, H_z, W_z]
        else:
            raise ValueError("z should be a list or a tensor")
        _, T_z, C_z, H_z, W_z = z.shape
        z = z.flatten(0, 1) # [B*T_z, C_z, H_z, W_z]
        ...
        z = z + self.pos_embed_z # (B*T_z, N, C) + (1, N, C) = (B*T_z, N, C)
