...
def calc_iou(pred_bb, anno_bb):
    ...
    # Calculate top-left and bottom-right coordinates of the intersection
    tl = np.maximum(pred_bb[:2], anno_bb[:2])  # Top-left corner of intersection
    br = np.minimum(pred_bb[:2] + pred_bb[2:] - 1.0, anno_bb[:2] + anno_bb[2:] - 1.0)  # Bottom-right corner of intersection
    
    # Calculate width and height of the intersection area
    sz = np.maximum(br - tl + 1.0, 0)  # Clamp values to ensure non-negative width and height

    # Calculate area of the intersection
    intersection = np.prod(sz)

    # Calculate area of each bounding box
    pred_area = np.prod(pred_bb[2:])
    anno_area = np.prod(anno_bb[2:])

    # Calculate area of the union
    union = pred_area + anno_area - intersection
...
def giou_loss(boxes1, boxes2, mask=None):
    """
    ...
    # print(giou,iou,'opppp')
    if mask is not None:
        if mask.sum() == 0:
            return torch.tensor(0.0, requires_grad=True, device=giou.device), torch.tensor(0.0, requires_grad=True, device=giou.device)
        giou = giou * mask
        giou = giou[mask!=0]
        iou = iou * mask
        iou = iou[mask!=0]
    #     print(giou,iou,mask,'opdfdsghfppp')
