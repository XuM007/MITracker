...
def get_scale(image_size, resized_size):
    w, h = image_size # 1032, 776
    w_resized, h_resized = resized_size # 800, 608
    if w / w_resized < h / h_resized: # w/w_resize = 1.29, h/h_size = 1.27
        w_pad = h / h_resized * w_resized
        h_pad = h
    else:
        w_pad = w # 1032
        h_pad = w / w_resized * h_resized # 784
    scale = np.array([w_pad / 200.0, h_pad / 200.0], dtype=np.float32) # 5.16, 3.92
