...
class TrackingSampler(torch.utils.data.Dataset):
    ...
    def __init__(self, datasets, p_datasets, samples_per_epoch, max_gap,
                 num_search_frames, num_template_frames=1, processing=no_processing, frame_sample_mode='causal', train_cls=False, pos_prob=0.5):
        """
        args:
    ...
    def getitem(self):
        ...
        while not valid:
            ...
            if is_video_dataset:
                ...
                if self.frame_sample_mode == 'causal':
                    ...
                    while search_frame_ids is None:
                        ...
                        # base_frame_id is the last frame of the template, prev_frame_ids are the previous frames
            ...
            data = TensorDict({'template_images': template_frames,
                            ...
                            'search_visible': search_anno['visible'] if isinstance(seq_info_dict, list) else None,
                            'dataset': dataset.get_name(),
                            'scene_name': meta_obj_train.get('scene_name') if 'scene_name' in meta_obj_train else None,
                            'test_class': meta_obj_test.get('object_class_name'),
                            'exp_str': meta_obj_train.get('exp_str') if 'exp_str' in meta_obj_train else None,
                            'seq_view_id': search_anno['seq_view_id'] if isinstance(seq_info_dict, list) else None,
                            'bev_xys': meta_obj_test.get('bev_xys') if 'bev_xys' in meta_obj_test else None,
                            'cube_xyz': meta_obj_test.get('cube_xyz') if 'cube_xyz' in meta_obj_test else None,
                            'bev_valid': meta_obj_test.get('bev_valid') if 'bev_valid' in meta_obj_test else None,
                            })
            if 'bev_valid' in meta_obj_test:
                if meta_obj_test['bev_valid']:
    ...
    def sample_seq_from_dataset(self, dataset, is_video_dataset):
        ...
        while not enough_visible_frames:
            ...
            if isinstance(seq_info_dict, dict): 
                ...
                valid = visible
            elif isinstance(seq_info_dict, list): # multiview read, seq_info_dict = [seq_info1, seq_info2, ...]
                all_visibile = None
                for seq_info in seq_info_dict:
                    ...
                    if not enough_visible_frames:
                        break
                    if all_visibile is None:
                        all_visibile = visible
                    else:
                        all_visibile = all_visibile & visible
                valid = all_visibile
            else:
                raise 'TypeError: seq_info_dict from dataset is wrong!'
        return seq_id, valid, seq_info_dict
