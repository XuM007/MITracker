...
import numpy as np
from lib.train.data.transforms import get_affine_transform
import lib.train.data.processing as ppp
...
def sample_target(im, target_bb, search_area_factor, output_sz=None, mask=None):
    ...
    # Crop target
    im_crop = im[y1 + y1_pad:y2 - y2_pad, x1 + x1_pad:x2 - x2_pad, :] # crop image [x1+x1_pad, y1+y1_pad]  [x2-x2_pad, y2-y2_pad]
    if mask is not None:
    ...
    crop_bbox = torch.tensor([x1, y1, crop_sz, crop_sz])

    r = 0
    c = np.array([im_crop_padded.shape[0] / 2.0, im_crop_padded.shape[1] / 2.0]) # center
    s = np.array([im_crop_padded.shape[0], im_crop_padded.shape[1]]) # scale
    resize_transform = torch.from_numpy(get_affine_transform(c, s, r, np.array([output_sz, output_sz]))).float()
    ...
    if output_sz is not None:
        ...
        if mask is None:
            return im_crop_padded, resize_factor, att_mask, crop_bbox, resize_transform
        mask_crop_padded = \
        F.interpolate(mask_crop_padded[None, None], (output_sz, output_sz), mode='bilinear', align_corners=False)[0, 0]
        return im_crop_padded, resize_factor, att_mask, mask_crop_padded, crop_bbox, resize_transform
    else:
        if mask is None:
...
def jittered_center_crop(frames, box_extract, box_gt, search_area_factor, output_sz, masks=None):
    ...
    if masks is None:
        ...
        frames_crop, resize_factors, att_mask, crop_bbox, resize_transform = map(list,zip(*crops_resize_factors))
        masks_crop = None
    else:
        crops_resize_factors = [sample_target(f, a, search_area_factor, output_sz, m)
                                for f, a, m in zip(frames, box_extract, masks)]
        frames_crop, resize_factors, att_mask, masks_crop, crop_bbox, resize_transform = zip(*crops_resize_factors)
    ...
    return frames_crop, box_crop, att_mask, masks_crop, crop_bbox, resize_transform
