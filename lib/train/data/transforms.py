...
class Transform:
    ...
    def __init__(self, *transforms):
        ...
        self._valid_inputs = ['image', 'coords', 'bbox', 'mask', 'att','cam_pose']
    ...
    def __call__(self, **inputs):
        ...
        out = {k: v for k, v in inputs.items() if k in self._valid_inputs} # dict_keys(['image', 'bbox']))
        ...
        return tuple(out[v] for v in var_names) #(out['image'], out['coords'], out['bbox'], out['mask'], out['att'], out['cam_pose') 
...
class TransformBase:
    ...
    def __call__(self, **inputs):
        ...
        if input_args.get('new_roll', True):
            rand_params = self.roll() # do or not
...
class ToTensorAndJitter(TransformBase):
    ...
    def transform_cam_pose(self, cam_pose, brightness_factor):
        pass
