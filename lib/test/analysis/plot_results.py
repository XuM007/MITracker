...
def get_plot_draw_styles():
    plot_draw_style = [{'color': (1.0, 0.0, 0.0), 'line_style': '-', 'marker_style': 'o'},
                       {'color': (0.0, 1.0, 0.0), 'line_style': '-', 'marker_style': 's'}, 
                       {'color': (0.0, 0.0, 1.0), 'line_style': '-', 'marker_style': 'd'},
                       {'color': (0.0, 0.0, 0.0), 'line_style': '-.', 'marker_style': '*'},
                       {'color': (1.0, 0.0, 1.0), 'line_style': '-.', 'marker_style': 'x'},
                       {'color': (0.0, 1.0, 1.0), 'line_style': '-.', 'marker_style': '+'},
                       {'color': (0.5, 0.5, 0.5), 'line_style': '-', 'marker_style': 'v'},
                       {'color': (136.0 / 255.0, 0.0, 21.0 / 255.0), 'line_style': '-', 'marker_style': '^'},
                       {'color': (1.0, 127.0 / 255.0, 39.0 / 255.0), 'line_style': '-', 'marker_style': '<'},
                       {'color': (0.0, 162.0 / 255.0, 232.0 / 255.0), 'line_style': '-.', 'marker_style': '>'},
                       {'color': (0.0, 0.5, 0.0), 'line_style': '-.', 'marker_style': '1'},
                       {'color': (1.0, 0.5, 0.2), 'line_style': '-.', 'marker_style': 'h'},
...
def get_tracker_display_name(tracker, isPlot=None):
...
def plot_draw_save(y, x, scores, trackers, plot_draw_styles, result_plot_path, plot_opts):
    ...
    # Plot settings
    font_size_legend = plot_opts.get('font_size_legend', 27)
    font_size = plot_opts.get('font_size', 40)
    font_size_axis = plot_opts.get('font_size_axis', 42)
    line_width = plot_opts.get('line_width', 4)
    if 'Restart' in plot_opts['xlabel']:
    ...
    for id, id_sort in enumerate(index_sort):
        if 'Restart' in xlabel:
            line = ax.plot(x[id_sort], y[id_sort],
                           marker=plot_draw_styles[index_sort.numel() - id - 1]['marker_style'],
                           markersize=26,
                           color=plot_draw_styles[index_sort.numel() - id - 1]['color'],
                           linestyle='')
        elif 'recovery' in plot_type:
            line_width = 5
    ...
    try:
        if 'Restart' in xlabel:
            ax.legend(plotted_lines[::-1], legend_text[::-1], loc=legend_loc, fancybox=False, edgecolor='black',
                    fontsize=font_size_legend, framealpha=0.8, ncol=1)
        else:
            ax.legend(plotted_lines[::-1], legend_text[::-1], loc=legend_loc, fancybox=False, edgecolor='black',
                    fontsize=font_size_legend, framealpha=0.8, ncol=2)
    except:
        pass
    ...
    if 'Restart' in xlabel:
        if not os.path.exists(f'{result_plot_path}_restart'):
            os.makedirs(f'{result_plot_path}_restart')
        fig.savefig('{}_restart/{}_plot.pdf'.format(result_plot_path, plot_type), dpi=300, format='pdf', transparent=True)
    else:
...
def check_and_load_precomputed_results(trackers, dataset, report_name, force_evaluation=False, **kwargs):
    ...
    if not check_eval_data_is_valid(eval_data, trackers, dataset):
        ...
        tracker_names = [{'name': t.name, 'param': t.parameter_name, 'run_id': t.run_id, 'disp_name': t.display_name, 'result_type': t.result_type}
                         for t in trackers]
...
def get_recovery_scale(seq_recovery_frames):
    trk_recovery_frames = []

    for key in seq_recovery_frames.keys():
        Count = Counter(seq_recovery_frames[key])
        sort_count = dict(sorted(Count.items(), key=operator.itemgetter(0)))
        scale_dict = {}
        pre_sum = 0
        for key in sort_count.keys():
            pre_sum += sort_count[key]
            scale_dict[key] = pre_sum
        scale_dict = {k: v * 100 / pre_sum for k, v in scale_dict.items()}
        trk_recovery_frames.append(scale_dict)
    return trk_recovery_frames
...
def plot_results(trackers, dataset, report_name, merge_results=False,
    ...
    if 'success' in plot_types:
        ...
        success_plot_opts = {'plot_type': 'success', 'legend_loc': 'lower left', 'xlabel': 'Overlap threshold',
                             'ylabel': 'Success rate (%)', 'xlim': (0, 1.0), 'ylim': (0, 100), 'title': 'Success plots on MVTrack (based on IoU)'}
    ...
    if 'prec' in plot_types:
        ...
        precision_plot_opts = {'plot_type': 'precision', 'legend_loc': 'lower right',
                               ...
                               'xlim': (0, 50), 'ylim': (0, 100), 'title': 'Precision plots on MVTrack'}
    ...
    if 'norm_prec' in plot_types:
        ...
        norm_precision_plot_opts = {'plot_type': 'norm precision', 'legend_loc': 'lower right',
                                    'xlabel': 'Normalized location error threshold', 'ylabel': 'Normalized precision (%)',
                                    'xlim': (0, 0.5), 'ylim': (0, 100), 'title': 'Normalized precision plots on MVTrack'}
    ...
    if 'recovery' in plot_types:
        seq_recovery_frames = eval_data['seq_recovery_frames']
        scale_dict = get_recovery_scale(seq_recovery_frames) #[trk_id]{dict}
        x_scale = 31
        x = torch.arange(x_scale)
        y = torch.zeros((len(scale_dict), x_scale))
        for trk_id in range(len(scale_dict)):
            for i in range(1, x_scale):
                if i not in scale_dict[trk_id].keys():
                    y[trk_id, i] = y[trk_id, i - 1]
                else:
                    y[trk_id, i] = scale_dict[trk_id][i]
        recovery_plot_opts = {'plot_type': 'Frame numbers needed for recovery', 'legend_loc': 'lower right',
                                'xlabel': 'Frame numbers needed for recovery', 'ylabel': 'Recovery rate (%)',
                                    'xlim': (0, x_scale-1), 'ylim': (0, 100), 'title': 'Recovery plots on MVTrack'}
        
        plot_draw_save(y, x, y[:, 10], tracker_names, plot_draw_styles, result_plot_path,
                        recovery_plot_opts)
        
    # ********************************  Restart Plot *********************************
    if 'restart' in plot_types:
        seq_restart = eval_data['seq_restart']
        all_ave_count = [[]for _ in range(len(trackers))]
        all_ave_max_len = [[]for _ in range(len(trackers))]
        for key in seq_restart.keys():
            import numpy as np
            trk_id = int(key.split('-')[-1])
            all_ave_count[trk_id]+=seq_restart[key][0]
            all_ave_max_len[trk_id]+=seq_restart[key][1]
        all_ave_count = torch.tensor(np.array(all_ave_count).mean(-1))
        all_ave_max_len = torch.tensor(np.array(all_ave_max_len).mean(-1))

        restart_plot_opts = {'plot_type': 'Robust tracking', 'legend_loc': 'upper right',
                                'xlabel': 'Restart count', 'ylabel': 'Max length',
                                    'xlim': (all_ave_count.min()-0.2, all_ave_count.max()+0.2), 'ylim': (all_ave_max_len.min()-20, all_ave_max_len.max()+20), 'title': 'Robust plot on MVTrack'}
        plot_draw_save(all_ave_max_len, all_ave_count, all_ave_max_len, tracker_names, plot_draw_styles, result_plot_path,
                        restart_plot_opts)
...
def print_results(trackers, dataset, report_name, merge_results=False,
    ...
    # ********************************  Restart Print *********************************
    if 'restart' in trackers[0].result_type:
        seq_restart = eval_data['seq_restart']
        all_ave_count = [[]for _ in range(len(trackers))]
        all_ave_max_len = [[]for _ in range(len(trackers))]
        for key in seq_restart.keys():
        ...
        all_ave_count = torch.tensor(np.array(all_ave_count).mean(-1))
        all_ave_max_len = torch.tensor(np.array(all_ave_max_len).mean(-1))
        scores['Restart count'] = all_ave_count
        scores['Restart max length'] = all_ave_max_len

    # Print
    tracker_disp_names = [get_tracker_display_name(trk, 1) for trk in tracker_names]
