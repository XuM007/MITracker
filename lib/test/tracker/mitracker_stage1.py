import numpy as np
from lib.models.mitracker import build_mitracker_stage1
...
class MITracker_stage1(BaseTracker):
    def __init__(self, params):
        super(MITracker_stage1, self).__init__(params)
        network = build_mitracker_stage1(params.cfg, training=False)
    ...
    def track(self, image, info: dict = None):
        ...
        self.frame_id += 1
        if self.frame_id > 1:
            self.state = info['previous_output']['target_bbox']
        x_patch_arr, resize_factor, x_amask_arr, crop_bbox, resize_transform = sample_target(image, self.state, self.params.search_factor,
        ...
        z_patch_arr, z_resize_factor, z_amask_arr, crop_bbox, resize_transform = sample_target(image, self.state, self.params.template_factor,
                                                    output_sz=self.params.template_size)
