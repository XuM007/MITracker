...
def trackerlist(name: str, parameter_name: str, dataset_name: str, run_ids = None, display_name: str = None,
    ...
    if run_ids is None or isinstance(run_ids, int):
        run_ids = [run_ids]
    return [Tracker(name, parameter_name, dataset_name, run_id, display_name, result_only, result_type) for run_id in run_ids]
...
class Tracker:
    ...
    def run_sequence(self, seq, debug=None, multiobj_mode=None, restart=False):
        ...
        output = self._track_sequence(tracker, seq, init_info, restart)
        return output

    def _track_sequence(self, tracker, seq, init_info, restart=False):
        # Define outputs
        ...
        for frame_num, frame_path in enumerate(seq.frames[1:], start=1):
            ...
            if fail_count > 10 and restart and seq.target_visible[frame_num]==1:       
                # the tracker will be restarted when the cumulative number of failures reaches 10       
                init_positions.append(frame_num)
            ...
            if restart and seq.target_visible[frame_num]==1: # R-OPE mechanism, and the target is visible, compute the overlap
                iou = calc_iou(out['target_bbox'], seq.ground_truth_rect[frame_num])
                if iou < 0.5:
                    fail_count += 1
                else:
                    fail_count = 0
        ...
        if restart:
            output['init_positions'] = init_positions
